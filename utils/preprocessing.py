...
def to_int(seq, max_length):
    ...
    index = tmp.size if tmp.size<1024 else 1024
    out[:index] = tmp[:index]
    return out

def loss_weight(len_seq, max_length):
...
def to_binary_mask(mask):
    ...
    binary_code = dict()
    for aa in aas: 
        code = np.zeros(3, dtype = np.int8)
        code[pos[aa]] = 1
        binary_code[aa] = code
    
    seq_coding = np.zeros((len(seq),3), dtype = np.int8)
    for i,aa in enumerate(seq): 
        code = binary_code.get(aa,np.zeros(3, dtype = np.int8))
        seq_coding[i,:] = code
    return seq_coding
